...
export function TicketDetail({ ticket }) {
  ...
  return <div className="space-y-7 pb-8">
    ...
    {(activeTicket.recommendedSolution || activeTicket.visibleSolutions?.length) && <DetailSection icon={Lightbulb} title="Tentativas antes do suporte"><div className="rounded-2xl border border-blue-200 bg-blue-50 p-4"><p className="text-sm font-semibold text-blue-950">Primeira solução recomendada: {activeTicket.recommendedSolution?.title || activeTicket.visibleSolutions?.[0]?.title}</p>{activeTicket.visibleSolutions?.length > 0 && <ol className="mt-3 space-y-2">{activeTicket.visibleSolutions.map((solution, index) => <li key={`${solution.id}-${solution.presentedAt || index}`} className="text-sm leading-6 text-blue-900"><strong>{solution.order || index + 1}. {solution.title}</strong>{solution.recommended ? " · recomendação inicial" : " · alternativa apresentada"}{solution.presentedAt ? ` · ${new Date(solution.presentedAt).toLocaleString("pt-BR")}` : ""}</li>)}</ol>}{activeTicket.solutionAttempts?.length > 0 ? <ul className="mt-4 space-y-2 border-t border-blue-200 pt-3">{activeTicket.solutionAttempts.map((attempt) => <li key={`${attempt.solutionId}-${attempt.at}`} className="text-sm leading-6 text-blue-900"><strong>{attempt.title}</strong>: {formatAttemptOutcome(attempt.outcome)} em {new Date(attempt.at).toLocaleString("pt-BR")}.{attempt.note ? ` Observação: ${attempt.note}` : ""}</li>)}</ul> : <p className="mt-3 text-sm text-blue-900">Nenhuma solução foi marcada como tentada antes da abertura deste chamado.</p>}</div></DetailSection>}
...
function formatAttemptOutcome(outcome) { return ({ resolveu: "resolveu", "nao-resolveu": "não resolveu", "nao-sei": "ainda não sabe", "nao-consegui": "não conseguiu tentar" })[outcome] || "foi registrada"; }
