...
export function TicketWorkflow({ ticket, onUpdate }) {
  ...
  function apply(result) { if (!result.ok) return setMessage(result.error); setMessage(""); onUpdate?.(result.ticket); }
  function sendComment() { apply(addTicketComment(ticket.id, comment, requestDetails)); setComment(""); setRequestDetails(false); }
  function changeStatus(status) { apply(updateTicketStatus(ticket.id, status)); }
  function sendEvaluation() { if (evaluation === null) return setMessage("Escolha se o problema foi resolvido."); apply(saveTicketEvaluation(ticket.id, evaluation, evaluationText)); }

  return <div className="space-y-7">
    {message && <p role="alert" className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-medium text-amber-800">{message}</p>}
