import { diagnosticTrees } from "../data/diagnosticTrees.js";
import { adaptQuestionToContext } from "./question-adaptation.js";
import { selectDiagnosticTreeId } from "./diagnostic-selection.js";

export { adaptQuestionToContext } from "./question-adaptation.js";
...
export function answerDiagnostic(tree, state, answerId) {
  ...
  const possibilities = answer.preservePossibilities
    ? state.possibilities
    : answer.keep?.length
  ...
  if (answer.finish && state.pendingResultId) {
    return { done: true, resultId: state.pendingResultId, state: { ...state, path, possibilities } };
  }
  if (answer.result && tree.completionNode) {
    return {
      done: false,
      state: { ...state, path, possibilities, nodeId: tree.completionNode, pendingResultId: answer.result },
    };
  }
...
export function buildDiagnosisResult(tree, diagnosticState, resultId) {
  ...
  return {
    ...
    impact: diagnosticState.path.find((step) => step.nodeId === tree.completionNode)?.answerId || "unknown",
    elapsedSeconds,
