...
export function calculateTicketQuality(result, evidence) {
  ...
  if (result?.treeId === "reports") {
    checks.push(
      [/relatorio|vendas|financeiro|estoque|clientes/.test(text), "nome do relatório"],
      [/periodo|mensal|semanal|diario|filtro|filial|empresa/.test(text), "período ou filtros usados"],
    );
    if (/export|baix|download|pdf|excel|csv/.test(text)) checks.push([/pdf|excel|csv|formato/.test(text), "formato de exportação"]);
    if (/valor|divergen|incorret|esperad/.test(text)) checks.push([/valor esperado|valor exibido|difer/.test(text), "exemplo do valor esperado e exibido"]);
  }
