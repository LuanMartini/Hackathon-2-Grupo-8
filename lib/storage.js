...
export function getCustomTickets() {
  ...
  try {
    ...
    return Array.isArray(tickets) ? tickets.filter((ticket) => ticket && typeof ticket.id === "string" && typeof ticket.userId === "string") : [];
  } catch { return []; }
}
...
export function loadOwnTicketsForCurrentProfile() {
  const profile = getDemoProfile();
  if (!profile) return [];
  return getCustomTickets().filter((ticket) => ticket.userId === profile.id);
}
...
export function addTicketComment(id, content, requestDetails = false) {
  ...
  return replaceCustomTicket(id, (ticket) => {
    if (!isSupportProfile(profile) && ticket.userId !== profile.id) throw new Error("Você não tem acesso a este chamado. Se precisar de ajuda, fale com a equipe de suporte.");
...
export function createTicketFromResult(result, analysis, actions, options = {}) {
  ...
  return {
    ...
    quality, similarTickets: options.similarTickets || [],
    recommendedSolution: options.recommendedSolution || null,
    visibleSolutions: options.visibleSolutions || [],
    solutionAttempts: options.solutionAttempts || [],
    caseEvaluation: options.caseEvaluation || null,
    comments: [], evaluation: null, statusHistory: [{ status: "Aberto", at: now, author: profile.name, role: profile.role }],
  };
}

// IMPORTANTE: anexos, comentários, tentativas de solução e permissões são locais neste MVP.
